...
This is a self-hosted MCP server. Point your client at the server you deploy:

- **Streamable HTTP (preferred):** `https://<your-host>/mcp`
- **Legacy SSE:** `https://<your-host>/sse`

The server supports two authentication modes:

- **`auth.mode: config`** — for local stdio runs. Credentials live in the YAML config (`green_api.instances`) or in `GREEN_API_INSTANCE_ID` + `GREEN_API_TOKEN` env vars.
- **`auth.mode: proxy`** — for networked HTTP transports. Each request carries credentials in HTTP headers (`Authorization: Basic <base64(instanceId:apiToken)>` or `X-Instance-Id` + `X-Api-Token`), or the server runs a built-in OAuth 2.0 Authorization Code + PKCE flow. In proxy mode `whatsapp_connect` is not required.
...
- `ui://qr` — interactive QR code widget for authorizing an instance
- `ui://contacts` — contacts list widget with search and contact details
